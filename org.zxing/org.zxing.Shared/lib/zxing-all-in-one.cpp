...
std::vector<Ref<ResultPoint> > MonochromeRectangleDetector::detect() {
    ...
    int deltaY = _max(1, height / (MAX_MODULES << 3));
    int deltaX = _max(1, width / (MAX_MODULES << 3));
...
//      }
    ...
    int* ITFReader::findGuardPattern(Ref<BitArray> row, int rowOffset, const int pattern[],
      ...
      const int patternLength = patternLen;
	  std::unique_ptr<int[]> counters(new int[patternLength]);
      ...
      for (int x = rowOffset; x < width; x++) {
        ...
        if (pixel ^ isWhite) {
          ...
          if (counterPosition == patternLength - 1) {
            if (patternMatchVariance(counters.get(), patternLength, pattern,
    ...
    int ITFReader::decodeDigit(int counters[], int countersLen){
      ...
      for (int i = 0; i < max; i++) {
		std::unique_ptr<int[]> pattern(new int[countersLen]);
        for(int ind = 0; ind<countersLen; ind++){
          pattern[ind] = PATTERNS[i][ind];
        }
        unsigned int variance = patternMatchVariance(counters, countersLen, pattern.get(),
            MAX_INDIVIDUAL_VARIANCE);
        if (variance < bestVariance) {
          bestVariance = variance;
          bestMatch = i;
    ...
    bool UPCEANReader::findGuardPattern(Ref<BitArray> row, int rowOffset, bool whiteFirst,
      ...
      for (int x = rowOffset; x < width; x++) {
        ...
        if (pixel ^ isWhite) {
          ...
          if (counterPosition == patternLength - 1) {
            if (patternMatchVariance(counters.get(), countersCount, pattern,
                MAX_INDIVIDUAL_VARIANCE) < MAX_AVG_VARIANCE) {
              *start = patternStart;
              *end = x;
              return true;
